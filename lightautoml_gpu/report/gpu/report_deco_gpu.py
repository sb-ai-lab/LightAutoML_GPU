...
from lightautoml_gpu.addons.uplift.metalearners import TLearner, XLearner
from lightautoml_gpu.addons.uplift.utils import _get_treatment_role
...
class ReportDeco:
    """
    Decorator to wrap :class:`~lightautoml_gpu.automl.base.AutoML` class to generate html report on ``fit_predict`` and ``predict``.
...
class ReportDecoWhitebox(ReportDeco):
    """
    Special report wrapper for :class:`~lightautoml_gpu.automl.presets.whitebox_presets.WhiteBoxPreset`.
    Usage case is the same as main
    :class:`~lightautoml_gpu.report.report_deco.ReportDeco` class.
    It generates same report as :class:`~lightautoml_gpu.report.report_deco.ReportDeco` ,
    ...
    Difference:
        ...
        - :class:`~lightautoml_gpu.automl.presets.whitebox_presets.WhiteBoxPreset`
...
class ReportDecoNLP(ReportDeco):
    """
    Special report wrapper for :class:`~lightautoml_gpu.automl.presets.text_presets.TabularNLPAutoML`.
    Usage case is the same as main
    :class:`~lightautoml_gpu.report.report_deco.ReportDeco` class.
    It generates same report as :class:`~lightautoml_gpu.report.report_deco.ReportDeco` ,
...
class ReportDecoUplift(ReportDeco):
    ...
    def _generate_test_subsection(self, data, prefix, title):
        ...
        if self.task == "binary":
            ...
            self._inference_content["bins_preds"] = prefix + "_bins_preds_{}.png".format(
                self._n_test_sample
            )
            self._inference_content[
                "preds_distribution_by_bins"
            ...
            self._inference_content["error_hist"] = prefix + "_error_hist_{}.png".format(
                self._n_test_sample
            )
            self._inference_content[
                "scatter_plot"
